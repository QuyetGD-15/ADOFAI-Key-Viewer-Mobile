# 🌍 Language / Ngôn ngữ
[Tiếng Việt](README_VN.md) | [English](README.md)

---

# 🎹 ADOFAI Key Viewer Mobile

An ultra-lightweight, ultra-low latency overlay tool designed specifically for ADOFAI and other rhythm games on Android. Bypass Game Turbo limitations to capture every single keypress.

## ✨ Key Features
* **🚀 Shizuku Core:** Reads input events (`getevent`) directly from the system, completely eliminating OS latency. Zero dropped inputs even at high speeds.
* **Bypass Game Turbo:** Smoothly displays over system game optimization overlays thanks to the AccessibilityService architecture.
* **📊 KPS & Total Tracker:** Monitor Keys Per Second (KPS) and total clicks in real-time.
* **Minimalist UI:** Anti-glare design (Drop Shadow) ensures the keyrain always stands out, even on bright white Flashbang maps.

<img width="386" height="386" alt="Screenshot 2026-06-11 154833" src="https://github.com/user-attachments/assets/52af2add-19fe-420b-9fde-9b74e2908e44" />

## 📥 Installation (Beta)
1. Download the latest APK from the [Releases](https://github.com/QuyetGD-15/ADOFAI-Key-Viewer-Mobile/releases) section.
2. Install the application.
3. Grant "Display over other apps" permission.
4. Enable Shizuku (required for the best zero-latency experience).

---
**Developed with 💖 by [quyetgd]**
